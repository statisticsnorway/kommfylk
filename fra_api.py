...
def kommuner_fra_api(aar=None, 
    ...
    if navn:
        out = pd.DataFrame(data = {'KOMMUNENR':list(kommuner.code), 
                                   'NAVN':list(kommuner.name)})
...
def kommunenavn(kommune=None, aar=None):
    """
    ...
    kommunenavn(df.KOMMUNENR) -> pandas-kolonne med kommunenavn
    """
    ...
    def finn_kommnavn(kommune, kommuneordbok, aar):
        ...
        if aar is None:
            ...
            try:
                ...
                raise ValueError(f"Finner ikke kommune '{kommune}' i {aar}")
    ...
    if isinstance(kommune, (str, int, float)):
    ...
    elif isinstance(kommune, (list, tuple)):
...
def kommnavn_til_fylknavn(aar=None, samisk=False):
    ...
    fylkesnavn = kommuner.KOMMUNENR.str[:2].map(fylker.set_index("FYLKE").NAVN)
...
def fylkesnavn(fylke=None, # fylkesnummer (evt. kommunenr) som string, tall, liste eller pandas-kolonne
    ...
    fylkesnavn(df.FYLKE) -> pandas-kolonne med fylkesnavn
    """
    ...
    elif isinstance(fylke, (list, tuple)):
...
def kommunenummer(kommune=None, aar = None):
    """
